# Claude Code Project Instructions — ignore-kotlin

## Project Overview

This is **ignore-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`ignore`](https://crates.io/crates/ignore).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/ignore/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/ignore>
